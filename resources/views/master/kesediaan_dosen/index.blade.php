<x-app-layout title="Data Kesediaan Dosen">
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">Data Kesediaan Dosen Menguji</h2>
                <p class="text-sm text-slate-500 mt-1">Informasi ketersediaan hari, tanggal, dan rentang jam dosen untuk penjadwalan ujian sempro dan skripsi.</p>
            </div>
        </div>
        ...
        @if($activePeriode)
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm mb-6">
                <div class="flex items-center gap-3 pb-4 border-b border-slate-100 mb-5">
                    <div class="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-lg border border-indigo-100 shadow-xs shrink-0">
                        ⚙️
                    </div>
                    <div>
                        <h3 class="text-base font-extrabold text-slate-900">Pengaturan Akses Form Kesediaan Dosen</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Konfigurasi visibilitas dan status penguncian form kesediaan menguji untuk dosen pada periode aktif: <strong class="text-indigo-600">{{ $activePeriode->nama_periode }}</strong></p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('master.kesediaan-dosen.settings') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Status Akses Form Dosen</label>
                            <select name="show_form_kesediaan" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-semibold focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 text-slate-800 transition-all cursor-pointer">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Status Penguncian Input</label>
                            <select name="lock_form_kesediaan" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-semibold focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 text-slate-800 transition-all cursor-pointer">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm mb-6">
            <div class="flex items-center gap-3 pb-4 border-b border-slate-100 mb-5">
                <div class="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center font-bold text-lg border border-violet-100 shadow-xs shrink-0">
                    🔑
                </div>
                <div>
                    <h3 class="text-base font-extrabold text-slate-900">Manajemen Hak Akses Pengisian Dosen</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Aktifkan atau nonaktifkan hak pengisian form kesediaan menguji secara individu untuk setiap dosen.</p>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($dosens as $d)
                    <div class="bg-slate-50/50 hover:bg-slate-50 border border-slate-200/80 rounded-2xl p-4 transition-all duration-200 flex items-center justify-between gap-4 shadow-xs">
                        <div class="min-w-0">
                            <h4 class="font-bold text-slate-800 text-sm leading-snug truncate" title="{{ $d->nama_dosen }}">{{ $d->nama_dosen }}</h4>
                            <span class="inline-block mt-1 px-2 py-0.5 rounded bg-slate-200/60 text-slate-600 font-mono text-[10px]">NIDN: {{ $d->nidn }}</span>
                            <div class="mt-2">
        ...
        {{-- Toolbar / Filters --}}
        <form method="GET" action="{{ route('master.kesediaan-dosen.index') }}" class="bg-white rounded-2xl border border-slate-200/80 p-4 mb-6 shadow-sm flex flex-wrap items-center gap-4">
            <div class="relative flex-1 min-w-[240px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama dosen, NIP, keterangan…" class="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                <svg class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            </div>

            <select name="per_page" class="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 cursor-pointer focus:ring-2 focus:ring-indigo-500" onchange="this.form.submit()">
            ...
            <select name="dosen_id" class="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 cursor-pointer focus:ring-2 focus:ring-indigo-500" onchange="this.form.submit()">
            ...
            <select name="periode_id" class="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 cursor-pointer focus:ring-2 focus:ring-indigo-500" onchange="this.form.submit()">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($kesediaans as $item)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-800">{{ $item['dosen']->nama_dosen ?? '-' }}</div>
                                    @if($item['dosen'] && $item['dosen']->nidn)
                                        <div class="mt-0.5"><span class="px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-mono text-[10px]">NIDN: {{ $item['dosen']->nidn }}</span></div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1.5 max-w-md">
                                        @foreach($item['dates'] as $date)
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-bold whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">
                                    @if($item['wave'])
                                        <span class="text-xs font-bold text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-md border border-indigo-200 inline-block">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-12 text-slate-400">
                                    ...
                                    <div class="font-bold text-slate-600">Belum Ada Data Kesediaan Dosen</div>
                                    <div class="text-xs text-slate-400 mt-1">Dosen akan mengisi kesediaan menguji melalui dashboard portal dosen.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
