<x-app-layout title="Master Dosen">
    ...
    <div x-data="{ 
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-4 mb-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <form method="GET" action="{{ route('master.dosen.index') }}" class="w-full sm:w-80 relative">
                <input type="text" 
                       name="search" 
            ...
            <div class="text-xs font-semibold text-slate-500">
                Total: <span class="font-extrabold text-slate-900">{{ $dosens->total() }}</span> Dosen
            </div>
        </div>
        ...
        <div x-show="createModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block relative z-10 w-full max-w-md my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-2xl shadow-xl">
                    <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                        <h3 class="text-lg font-extrabold text-slate-900">Tambah Dosen Baru</h3>
                    ...
                    <form method="POST" action="{{ route('master.dosen.store') }}" class="p-6 space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 mb-1">NIDN <span class="text-rose-500">*</span></label>
        ...
        <div x-show="editModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block relative z-10 w-full max-w-md my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-2xl shadow-xl">
                    <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                        <h3 class="text-lg font-extrabold text-slate-900">Edit Data Dosen</h3>
                    ...
                    <form :action="'/master/dosen/' + editDosen.id" method="POST" class="p-6 space-y-4">
                        @csrf
                        @method('PUT')
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 mb-1">NIDN <span class="text-rose-500">*</span></label>
        ...
        <div x-show="deleteModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block relative z-10 w-full max-w-sm my-8 overflow-hidden text-center align-middle transition-all transform bg-white rounded-2xl shadow-xl p-6">
                    ...
                    <form :action="'/master/dosen/' + deleteDosen.id" method="POST" class="flex items-center justify-center gap-3">
                        @csrf
                        @method('DELETE')
