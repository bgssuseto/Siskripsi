...
@endphp
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200/80 flex flex-col justify-between">
            <div>
                <h2 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Status Pendaftaran Sempro</h2>
                @if($activeWave)
                    <div class="flex items-center gap-3">
                        <span class="w-3 h-3 rounded-full bg-emerald-500 animate-pulse"></span>
                ...
                <div class="mt-4">
                    ...
                    @elseif($mySidang->verifikasi_status === 'ditolak')
                        <button @click="regModal = true" class="w-full sm:w-auto px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl transition-all shadow-md hover:shadow-lg shadow-rose-600/10">
                            Revisi Pendaftaran Seminar Proposal
                        </button>
                    @else
    ...
    {{-- True Screen-Centered Pop-Up Modal Pendaftaran Seminar Proposal --}}
    @if($activeWave)
    <div x-show="regModal" 
        ...
        <div @click.away="regModal = false" class="bg-white rounded-2xl w-full max-w-[400px] shadow-2xl border border-slate-100 overflow-hidden text-left my-auto transform transition-all relative z-50">
            <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-sm font-bold shadow-sm">
                        📝
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">Form Pendaftaran Seminar Proposal</h3>
            ...
            <form id="form-daftar-sempro" method="POST" action="{{ route('mahasiswa.daftar.store') }}" enctype="multipart/form-data" class="p-4 space-y-3">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="nim" value="{{ $mySidang->nim ?? $user->nim }}" required placeholder="NIM..." class="w-full px-2.5 py-1 border border-slate-300 rounded-lg text-xs focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 font-semibold bg-slate-50/50">
                    </div>
                ...
                <div>
                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Judul Proposal <span class="text-rose-500">*</span></label>
                    <textarea name="judul_skripsi" required rows="2" placeholder="Masukkan judul proposal tugas akhir Anda..." class="w-full px-2.5 py-1 border border-slate-300 rounded-lg text-xs focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 font-medium leading-relaxed">{{ $mySidang->judul_skripsi ?? '' }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="dosen_pembimbing_utama_id" required class="w-full px-2.5 py-1 border border-slate-300 rounded-lg text-xs focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 font-semibold cursor-pointer bg-white">
                    ...
                    <div>
                        ...
                        <select name="dosen_pembimbing_pendamping_id" class="w-full px-2.5 py-1 border border-slate-300 rounded-lg text-xs focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 font-semibold cursor-pointer bg-white">
                ...
                <div x-data="{
                    ...
                    <div class="relative border border-dashed rounded-xl p-2.5 text-center transition-all duration-200 cursor-pointer bg-slate-50/50 hover:bg-indigo-50/20"
                         :class="isDragging ? 'border-indigo-500 bg-indigo-50/50 shadow-inner' : (fileName ? 'border-emerald-400 bg-emerald-50/20' : 'border-slate-300 hover:border-indigo-500')"
                        ...
                        <template x-if="!fileName">
                            <div class="flex items-center justify-center gap-2">
                                <div class="w-6 h-6 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs shrink-0 border border-indigo-100">
                                    📁
                                </div>
                                <div class="text-left">
                    ...
                    <template x-if="!fileName && existingFile">
                        <div class="mt-1 flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-indigo-50 border border-indigo-100 text-indigo-700 text-[10px] font-bold">
                            <span>📄 Berkas Terunggah:</span>
                            <a :href="existingFile" target="_blank" class="text-indigo-600 hover:underline font-extrabold" @click.stop>
                                Lihat Berkas ↗
                            </a>
                        </div>
                    </template>
                ...
                <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-[11px] rounded-lg transition-all shadow-sm flex items-center gap-1">
    ...
    <div x-show="editBuktiModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/50 backdrop-blur-md" x-cloak>
        <div @click.away="editBuktiModal = false" class="bg-white rounded-3xl w-full max-w-[400px] shadow-2xl border border-slate-100 overflow-hidden text-left my-8 transform transition-all">
            ...
            <form id="form-edit-bukti-sempro" method="POST" :action="editBuktiAction" enctype="multipart/form-data" class="p-7 space-y-5">
                @csrf
                <div x-data="{
                        isDragging: false,
                        fileName: '',
...
<script>
            ...
            try {
                ...
                if (res.ok && data.success) {
                    ...
                    if (alpineEl && alpineEl._x_dataStack) {
                        Alpine.evaluate(alpineEl, 'regModal = false');
                    }
    ...
    const formEditBukti = document.getElementById('form-edit-bukti-sempro');
    if (formEditBukti) {
        formEditBukti.addEventListener('submit', async function(e) {
            e.preventDefault();
            // Get action from Alpine :action binding
            const action = this.getAttribute('action') || this.action;
            const btn = this.querySelector('[type=submit]'), orig = btn.innerHTML;
            btn.disabled = true; btn.innerHTML = '⏳ Menyimpan…';
            try {
                const res = await fetch(action, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF_MAHASISWA, 'Accept': 'application/json' }, body: new FormData(this) });
                const data = await res.json();
                if (res.ok && data.success) {
                    showToastMhs('success', data.message || 'Bukti pembayaran berhasil diperbarui!');
                    setTimeout(() => location.reload(), 1200);
                } else {
                    showToastMhs(res.status === 422 ? 'warning' : 'error', data.message || 'Terjadi kesalahan.');
                }
