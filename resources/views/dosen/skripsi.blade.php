<x-app-layout title="Jadwal Sidang Skripsi">
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">Jadwal Sidang Skripsi</h2>
        <p class="text-sm text-slate-500 mt-1">Daftar sidang pertanggungjawaban skripsi mahasiswa di mana Anda bertindak sebagai pembimbing atau tim penguji.</p>
    </div>

    <!-- Filters & Search Toolbar -->
    <form method="GET" action="{{ route('dosen.jadwal.skripsi') }}" class="flex flex-wrap items-center gap-3 bg-white p-4 rounded-3xl border border-slate-200/80 shadow-sm mb-6">
        <!-- Search bar -->
        ...
        <div class="flex items-center gap-2">
            ...
            @if(request()->hasAny(['search','tanggal','status']))
                <a href="{{ route('dosen.jadwal.skripsi') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-200 transition-all">
                    Reset
                </a>
            @endif
        </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-16 text-center text-slate-500">
                                    ...
                                    <h3 class="font-extrabold text-slate-800 text-base">Tidak Ada Jadwal Sidang</h3>
